  ...
  return <main style={{ padding: 20, fontFamily: "Arial, sans-serif" }}><div style={{ maxWidth: 1150, margin: "0 auto" }}><Link href="/">← Back to main pricing app</Link><h1>Display / Tradeshow Products</h1>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: 14 }}>
      <section style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 10 }}>
        ...
        <div style={{ marginTop: 10, display: "grid", gap: 6 }}>{filteredProducts.map((p)=><button key={p.id} onClick={()=>setSelectedProductId(p.id)} style={{textAlign:"left"}}>{p.name}</button>)}</div>
      </section>

      <section style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 10 }}>
        {!selectedProduct ? <div>No products available.</div> : <>
          <h3>{selectedProduct.name}</h3>
          <div style={{ color: "#475569", fontSize: 12 }}>{selectedProduct.subcategory}</div>
