...
export default function Embroidery({ onSummaryChange, isAdminView = false }) {
  ...
  const calculatedRetail=apparelRetail+embroiderySubtotal+handlingRetail+digitizingFees; const targetRetail=n(targetRetailPrice); const retail=targetRetail>0?targetRetail:calculatedRetail; const cost=apparelCost+(embroideryEachDirect*totalGarments)+handlingDirect+digitizingFees;
  ...
  {isAdminView && <><label>Target Retail Price</label><input style={input} placeholder="Optional override / what-if price" value={targetRetailPrice} onChange={(e)=>setTargetRetailPrice(e.target.value.replace(/[^0-9.]/g,""))} /></>}
