...
const DTG_ALLOWED_SIZES = ["S", "M", "L", "XL", "2XL", "3XL"];
const DTG_ALLOWED_COLORS = ["White", "Black"];
const DTG_REQUIRED_STYLE = "BC3001";
...
const n = (v) => Number.parseFloat(String(v ?? "").replace(/[^0-9.-]/g, "")) || 0;
const tier = (q) => QTY_TIERS.reduce((a, t) => (q >= t ? t : a), QTY_TIERS[0]);
...
export default function ScreenPrinting({ onSummaryChange, isAdminView = false }) {
  ...
  const styles = useMemo(() => {
    const m = new Map();
    rows.forEach((r) => {
      const k = `${r.style}__${r.title}`;
      if (!m.has(k)) m.set(k, { key: k, style: r.style, title: r.title, rows: [] });
      m.get(k).rows.push(r);
    });
    return m;
  }, [rows]);
  ...
  const pick = (id, key) => {
    const g = styles.get(key);
    setLI(id, (x) => ({ ...x, styleKey: key, search: g ? `${g.style} — ${g.title}` : "", open: false, color: x.color && g?.rows.find((r) => r.color === x.color) ? x.color : "" }));
  };

  const quick = (id, style) => {
    const g = [...styles.values()].find((s) => s.style.toLowerCase() === style.toLowerCase());
    if (!g) return;
    const colors = [...new Set(g.rows.map((r) => r.color).filter(Boolean))];
    const black = colors.find((c) => c.toLowerCase() === "black") || colors.find((c) => c.toLowerCase().includes("black")) || colors[0] || "";
