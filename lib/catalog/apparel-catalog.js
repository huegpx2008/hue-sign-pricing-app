...
export async function loadApparelCatalog(options = {}) {
  ...
  if (typeof window !== "undefined" && typeof fetch === "function") {
    ...
    const { fs, resolvedPath } = await getServerApparelCatalogFile(options);
...
export async function getServerApparelCatalogFile(options = {}) {
  const dynamicImport = new Function("specifier", "return import(specifier)");
  const fs = await dynamicImport("node:fs/promises");
  const nodePath = await dynamicImport("node:path");
  const cwd = typeof process !== "undefined" ? process.cwd() : ".";
  const resolvedPath = nodePath.resolve(cwd, options.serverPath || SERVER_CATALOG_PATH);
  return { fs, resolvedPath };
}

export async function getServerApparelCatalogStatus(options = {}) {
  try {
    const { fs, resolvedPath } = await getServerApparelCatalogFile(options);
    ...
    return {
      ...
      errorMessage: error?.message || "Unable to inspect apparel catalog file.",
    };
  }
}
...
export function normalizeCatalogSizeQuantities(style, color, sizes = {}, catalog) {
  return Object.fromEntries(Object.entries(sizes).map(([size, quantity]) => {
    const catalogRow = findVariantSize(style, color, size, catalog);
    return [catalogRow?.size || normalizeText(size), Number(quantity)];
  }));
}
...
export function sortApparelSizes(sizes = []) {
  ...
  for (const size of sizes) {
    const supplierSize = normalizeText(size);
    const normalizedSize = normalizeSize(size);
    if (normalizedSize && !uniqueSizes.has(normalizedSize)) uniqueSizes.set(normalizedSize, supplierSize);
  }
  return [...uniqueSizes.values()].sort((a, b) => {
...
export function calculateApparelLineCost(lineItem, catalog, options = {}) {
  ...
  const requestedSizes = Object.keys(sizeQty);
  const sizes = options.sizes || (mode === "embroidery"
    ? sortApparelSizes((styleGroup?.rows || []).map((row) => row.size))
    : (requestedSizes.length ? requestedSizes : DEFAULT_SCREEN_SIZES));
