...
function normalizeSizeQuantities(input = {}) {
  return {
    XS: input.XS ?? input.qtyXs ?? 0,
    S: input.S ?? input.qtyS ?? 0,
    M: input.M ?? input.qtyM ?? 0,
    L: input.L ?? input.qtyL ?? 0,
    XL: input.XL ?? input.qtyXl ?? 0,
    "2XL": input["2XL"] ?? input.qty2xl ?? 0,
    "3XL": input["3XL"] ?? input.qty3xl ?? 0,
    "4XL": input["4XL"] ?? input.qty4xl ?? 0,
    "5XL": input["5XL"] ?? input.qty5xl ?? 0,
  };
}
...
export function calculateDtfPricing(rawInput = {}, catalog) {
  ...
  const totalGarmentQty = input.dtfMode === "dtfOnly"
    ...
    : (input.bringYourOwnApparel ? Math.max(0, Math.floor(toNumber(input.byoaTransferQty))) : (
      toNumber(sizeQuantities.XS)
      + toNumber(sizeQuantities.S)
      + toNumber(sizeQuantities.M)
      + toNumber(sizeQuantities.L)
      + toNumber(sizeQuantities.XL)
      + toNumber(sizeQuantities["2XL"])
      + toNumber(sizeQuantities["3XL"])
      + toNumber(sizeQuantities["4XL"])
      + toNumber(sizeQuantities["5XL"])
    ));
  ...
  return {
    ...
    sizeQuantities: input.bringYourOwnApparel ? {} : {
      XS: toNumber(sizeQuantities.XS),
      S: toNumber(sizeQuantities.S),
      M: toNumber(sizeQuantities.M),
      L: toNumber(sizeQuantities.L),
      XL: toNumber(sizeQuantities.XL),
      "2XL": toNumber(sizeQuantities["2XL"]),
      "3XL": toNumber(sizeQuantities["3XL"]),
      "4XL": toNumber(sizeQuantities["4XL"]),
      "5XL": toNumber(sizeQuantities["5XL"]),
    },
